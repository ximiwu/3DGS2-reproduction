...
renderCUDA(
	...
	float4* __restrict__ dL_dmean2D_2,

	// for dL_dconic2D_2, a 4*4 form hessian matrix looks like:
	// [a,b,c,d]
	// [e,f,g,h]
	// [i,j,k,l]
	// [m,n,o,p]
	// it is symmetric and row3、col3 are zeros,so only elements a、b、d、f、h、p are needed

	float4* __restrict__ dL_dconic2D,
	float3* __restrict__ dL_dconic2D_2_abd,
	float3* __restrict__ dL_dconic2D_2_fhp,
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float term = (-1 + 2 * con_o.y * dx_2.z);
			atomicAdd(&dL_dmean2D_2[global_id].x, dL_dG * G * (con_o.x * con_o.x * dx_2.x + conic_y_2 * dx_2.y + con_o.x * term));
			atomicAdd(&dL_dmean2D_2[global_id].y, dL_dG * G * (conic_y_2 * dx_2.z + con_o.x * con_o.z * dx_2.z + con_o.y * (-1 + con_o.x * dx_2.x + con_o.z * dx_2.y)));
			atomicAdd(&dL_dmean2D_2[global_id].w, dL_dG * G * (conic_y_2 * dx_2.x + con_o.z * con_o.z * dx_2.y + con_o.z * term));
			...
			const float dxdxdydy = dx_2.z * dx_2.z;
			atomicAdd(&dL_dconic2D_2_abd[global_id].x, G * (dx_2.x * dx_2.x / 4));
			atomicAdd(&dL_dconic2D_2_abd[global_id].y, G * (dx_2.x * dx_2.z / 2));
			atomicAdd(&dL_dconic2D_2_abd[global_id].z, G * (dxdxdydy / 4));
			atomicAdd(&dL_dconic2D_2_fhp[global_id].x, G * (dxdxdydy / 2));
			atomicAdd(&dL_dconic2D_2_fhp[global_id].y, G * (dx_2.y * dx_2.z / 2));
			atomicAdd(&dL_dconic2D_2_fhp[global_id].z, G * (dx_2.y * dx_2.y / 4));
